...
## [Exercício] Redução da Qualidade

A partir de um determinado gatilho, eventos, pode ser iniciado o processo de gerenciamento de (...). Uma redução da qualidade de forma inesperada de um serviço de TI é conhecido, em ITIL®, como:

- [x] A) Incidente
  > Quando um evento não esperado reduz a qualidade de um serviço, este evento é considerado um incidente. Reduções esperadas ou planejadas não são incidentes.
...
Uma redução da qualidade de um serviço de TI é conhecido, em ITIL®, como o **Incidente**, quando ocorre de forma inesperada.

<br>

## [Exercício] Gerenciamento de Incidente

A partir de um determinado gatilho, eventos, pode ser iniciado o processo de gerenciamento de incidentes. Um dos objetivos do gerenciamento de incidente é:

- [ ] A) Não permitir que o incidente seja descoberto pelo cliente.

- [ ] B) Assegurar que a probabilidade de o incidente ser descoberto pela alta administração seja mínimo.

- [x] C) Assegurar que o impacto do incidente no negócio seja minimizado e o serviço restaurado o mais rapidamente possível.
  > Incidente por não ser planejado dificilmente pode ser tratado antes que exista, mas mitigar riscos é uma boa ação ofensiva para que não aconteça.

- [ ] D) Garantir que ninguém perceba o incidente.

Assegurar que o impacto do incidente no negócio seja minimizado e o serviço restaurado o mais rapidamente possível.

<br>

## [Exercício] Evento

Evento pode ser definido como uma mudança de estado que possui significado para o gerenciamento de um item de configuração ou serviço de TI. Quando um evento ocorre:

- [ ] A) A equipe de operações de TI precisa tomar uma ação, mas não deve gerar ou registrar incidentes NUNCA.

- [ ] B) A equipe de projetos de TI precisa tomar uma ação, que pode levar à geração e registro de problemas.

- [x] C) A equipe de operações de TI precisa tomar uma ação, que pode levar à geração e registro de incidentes.
  > Nem todo evento é incidente, mas todo incidente é um evento. Precisamos saber distinguir eventos, incidentes e requisições de serviço para melhor aproveitamento na prova de Fundamentos.

- [ ] D) Nada deve ser feito. O processo logo será normalizado.

A equipe de operações de TI precisa tomar uma ação, que pode levar à geração e registro de incidentes.